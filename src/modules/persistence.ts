...
interface StatePersistence {
  save: (state: StateInterface["state"]) => void;
  load: () => Promise<StateInterface["state"]>;
}

class LocalStoragePersistence implements StatePersistence {
  save(state: StateInterface["state"]) {
    console.log(`will save`, state);
    localStorage.setItem("todoState", JSON.stringify(state));
  }

  async load(): Promise<StateInterface["state"]> {
    const todoState = localStorage.getItem("todoState");
    return JSON.parse(todoState);
  }
}

class FileSystemPersistence implements StatePersistence {
  save(state: StateInterface["state"]) {
  ...
  async load(): Promise<StateInterface["state"]> {
    try {
      const fileContent = (await upload()) as string;
...
export { StatePersistence, FileSystemPersistence, LocalStoragePersistence };
